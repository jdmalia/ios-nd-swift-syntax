...
for (food, good) in polarizingFoods {
    if good {
        print("\(food), yum!")
    } else {
        print("\(food), yuck!")
...
for (rapper, phoneNumber) in rapperPhoneNumbers {
    if phoneNumber.containsString("(415)") {
        rapperPhoneNumbers[rapper] = phoneNumber.stringByReplacingOccurrencesOfString("(415)", withString: "(510)")
    }
}
rapperPhoneNumbers
//: ## Switch Statements
